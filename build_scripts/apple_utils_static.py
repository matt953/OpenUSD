...
# Static TBB build targets (architecture-specific)
TARGET_MACOS_STATIC_ARM64 = "macOS-static-arm64"
TARGET_MACOS_STATIC_X86_64 = "macOS-static-x86_64"
TARGET_IOS_SIMULATOR_ARM64 = "iOS-simulator-arm64"
TARGET_IOS_SIMULATOR_X86_64 = "iOS-simulator-x86_64"
TARGET_VISIONOS_ARM64 = "visionOS-arm64"
TARGET_VISIONOS_SIMULATOR_ARM64 = "visionOS-simulator-arm64"
TARGET_VISIONOS_SIMULATOR_X86_64 = "visionOS-simulator-x86_64"

# Traditional embedded platforms (device builds)
EMBEDDED_PLATFORMS = [TARGET_IOS, TARGET_VISIONOS, TARGET_VISIONOS_ARM64]

# All simulator platforms
SIMULATOR_PLATFORMS = [
    TARGET_IOS_SIMULATOR_ARM64,
    TARGET_IOS_SIMULATOR_X86_64,
    TARGET_VISIONOS_SIMULATOR_ARM64,
    TARGET_VISIONOS_SIMULATOR_X86_64
]

# macOS static platforms
MACOS_STATIC_PLATFORMS = [
    TARGET_MACOS_STATIC_ARM64,
    TARGET_MACOS_STATIC_X86_64
]

# All platforms that need static TBB (big_iron.inc)
STATIC_TBB_PLATFORMS = (
    EMBEDDED_PLATFORMS +
    SIMULATOR_PLATFORMS +
    MACOS_STATIC_PLATFORMS
)
...
def TargetNeedsStaticTBB(context):
    """Returns True if the target requires static TBB (big_iron.inc)"""
    return context.buildTarget in STATIC_TBB_PLATFORMS

def TargetIsSimulator(context):
    """Returns True if the target is a simulator build"""
    return context.buildTarget in SIMULATOR_PLATFORMS

def TargetIsMacOSStatic(context):
    """Returns True if the target is a macOS static build"""
    return context.buildTarget in MACOS_STATIC_PLATFORMS

def GetTargetArchFromBuildTarget(context):
    """Extract architecture from build target name"""
    if context.buildTarget in [TARGET_MACOS_STATIC_ARM64, TARGET_IOS_SIMULATOR_ARM64,
                                TARGET_VISIONOS_ARM64, TARGET_VISIONOS_SIMULATOR_ARM64]:
        return TARGET_ARM64
    elif context.buildTarget in [TARGET_MACOS_STATIC_X86_64, TARGET_IOS_SIMULATOR_X86_64,
                                  TARGET_VISIONOS_SIMULATOR_X86_64]:
        return TARGET_X86
    return None
...
def GetTargetArch(context):
    ...
    # Handle architecture-specific static build targets
    if context.buildTarget in SIMULATOR_PLATFORMS or context.buildTarget in MACOS_STATIC_PLATFORMS:
        arch = GetTargetArchFromBuildTarget(context)
        return arch if arch else GetTargetArmArch()
...
def GetTargetArchPair(context):
    secondaryArch = None
    primaryArch = None  # Initialize to avoid unbound variable error

    if context.targetNative:
        primaryArch = GetHostArch()
    elif context.targetX86:
        primaryArch = TARGET_X86
    elif context.targetARM64:
        primaryArch = GetTargetArmArch()
    elif context.buildTarget in EMBEDDED_PLATFORMS:
        primaryArch = GetTargetArmArch()
    elif context.targetUniversal:
        primaryArch = GetHostArch()
    ...
    # Handle architecture-specific static build targets
    elif context.buildTarget in SIMULATOR_PLATFORMS or context.buildTarget in MACOS_STATIC_PLATFORMS:
        # Extract architecture from build target
        arch = GetTargetArchFromBuildTarget(context)
        primaryArch = arch if arch else GetTargetArmArch()

    # Fallback if still None
    if primaryArch is None:
        primaryArch = GetTargetArmArch()
...
def GetSDKRoot(context) -> Optional[str]:
    ...
    if context.buildTarget == TARGET_IOS:
        sdk = "iphoneos"
    elif context.buildTarget in [TARGET_VISIONOS, TARGET_VISIONOS_ARM64]:
        sdk = "xros"

    # Simulator builds
    elif context.buildTarget in [TARGET_IOS_SIMULATOR_ARM64, TARGET_IOS_SIMULATOR_X86_64]:
        sdk = "iphonesimulator"
    elif context.buildTarget in [TARGET_VISIONOS_SIMULATOR_ARM64, TARGET_VISIONOS_SIMULATOR_X86_64]:
        sdk = "xrsimulator"

    # macOS static builds use standard macOS SDK
    elif context.buildTarget in MACOS_STATIC_PLATFORMS:
        sdk = "macosx"
...
def ConfigureCMakeExtraArgs(context, args:List[str]) -> List[str]:
    ...
    # Set CMAKE_SYSTEM_NAME for cross-compilation targets
    if context.buildTarget == TARGET_IOS:
        system_name = "iOS"
    elif context.buildTarget in [TARGET_VISIONOS, TARGET_VISIONOS_ARM64]:
        system_name = "visionOS"
    elif context.buildTarget in [TARGET_IOS_SIMULATOR_ARM64, TARGET_IOS_SIMULATOR_X86_64]:
        system_name = "iOS"  # Simulators use iOS system name
    elif context.buildTarget in [TARGET_VISIONOS_SIMULATOR_ARM64, TARGET_VISIONOS_SIMULATOR_X86_64]:
        system_name = "visionOS"  # Simulators use visionOS system name
    ...
    # macOS static builds don't need CMAKE_SYSTEM_NAME (native builds)
    # but still need the SDK root set for consistency
    elif TargetIsMacOSStatic(context):
        args.append(f"-DCMAKE_OSX_SYSROOT={GetSDKRoot(context)}")
