...
def InstallZlib(context, force, buildArgs):
    with CurrentWorkingDirectory(DownloadURL(ZLIB_URL, context, force)):
        ...
        # for efficiency. Also remove the shared library build for static-only builds.
        PatchFile("CMakeLists.txt",
                ...
                ("set_target_properties(zlib PROPERTIES DEFINE_SYMBOL ZLIB_DLL)",
                    "# Shared library properties disabled"),
                ("set_target_properties(zlib PROPERTIES SOVERSION 1)",
                    "# Shared library properties disabled"),
                ("    set_target_properties(zlib PROPERTIES VERSION ${ZLIB_FULL_VERSION})",
                    "    # Shared library version disabled"),
                ("   set_target_properties(zlib zlibstatic PROPERTIES OUTPUT_NAME z)",
                    "   set_target_properties(zlibstatic PROPERTIES OUTPUT_NAME z)"),
                ("     set_target_properties(zlib PROPERTIES LINK_FLAGS \"-Wl,--version-script,\\\"${CMAKE_CURRENT_SOURCE_DIR}/zlib.map\\\"\")",
                    "     # Shared library link flags disabled"),
                ("    set_target_properties(zlib PROPERTIES SUFFIX \"1.dll\")",
                    "    # Shared library suffix disabled"),
                ("    install(TARGETS zlib zlibstatic",
                    "    install(TARGETS zlibstatic")])

        # Fix fdopen macro conflict with modern SDK headers
        # zlib 1.2.13's zutil.h defines fdopen as NULL for old platforms that
        # lacked it, but modern platforms (Linux/Windows/macOS/iOS) all have
        # fdopen, and the macro conflicts with system headers
        PatchFile("zutil.h",
                [("#        define fdopen(fd,mode) NULL /* No fdopen() */",
                    "/* fdopen is available on modern platforms - using system fdopen */")])

        # Add CMAKE_POLICY_VERSION_MINIMUM for CMake 4.x compatibility
        # zlib 1.2.13 requires cmake_minimum_required(VERSION 2.4.4) which is
        # incompatible with CMake 4.x which removed support for < 3.5
        # Also force static-only build to avoid creating .dylib files
        zlibArgs = buildArgs + [
            "-DCMAKE_POLICY_VERSION_MINIMUM=3.5",
            "-DBUILD_SHARED_LIBS=OFF"
        ]
        RunCMake(context, force, zlibArgs)
    ...
    with CurrentWorkingDirectory(sourceDir):
        ...
        if MacOS() and not context.targetWasm and context.buildTarget not in android_utils.ANDROID_PLATFORMS:
            ...
            if context.buildTarget in [apple_utils.TARGET_VISIONOS,
                                       ...
                                       apple_utils.TARGET_VISIONOS_SIMULATOR_ARM64,
                                       apple_utils.TARGET_VISIONOS_SIMULATOR_X86_64]:
                useTargetTriple = True
                ...
                # Add platform-specific version flags for iOS/macOS
                if context.buildTarget == apple_utils.TARGET_IOS:
                    macOSPlatformFlags = "-miphoneos-version-min=26.0"
                elif context.buildTarget in [apple_utils.TARGET_IOS_SIMULATOR_ARM64,
                                             apple_utils.TARGET_IOS_SIMULATOR_X86_64]:
                    macOSPlatformFlags = "-miphonesimulator-version-min=26.0"
                elif context.buildTarget in [apple_utils.TARGET_MACOS_STATIC_ARM64,
                                             apple_utils.TARGET_MACOS_STATIC_X86_64]:
                    macOSPlatformFlags = "-mmacosx-version-min=11.0"

            # For visionOS, use only -target (not -arch + version-min)
            # For iOS/macOS, combine -arch and version-min
            if useTargetTriple:
                compilerFlags = macOSPlatformFlags
            else:
                compilerFlags = macOSArch
                if macOSPlatformFlags:
                    compilerFlags = "{0} {1}".format(macOSArch, macOSPlatformFlags) if macOSArch else macOSPlatformFlags
        ...
        b2_settings = [
            ...
            'address-model=64',
            'link=static',  # CUSTOM PATCH: Build static libraries for iOS/visionOS
            'runtime-link=static',  # CUSTOM PATCH: Link runtime statically
            'threading=multi',
        ...
        if MacOS() and context.buildTarget not in android_utils.ANDROID_PLATFORMS:
            ...
            # Use the same compiler flags as bootstrap (includes platform flags for iOS/visionOS)
            if compilerFlags:
...
def InstallTBB_MacOS(context, force, buildArgs):
    with CurrentWorkingDirectory(DownloadURL(TBB_URL, context, force)):
        ...
        # Setup TBB build configs for visionOS
        if context.buildTarget in [apple_utils.TARGET_VISIONOS, apple_utils.TARGET_VISIONOS_ARM64]:
            ...
            PatchFile("build/visionos.clang.inc",
                      [("ifeq (ios,$(target))", "ifeq (visionos,$(target))"),
                       ("-miphoneos-version-min=", "-target arm64-apple-xros"),
                       ("iOS", "visionOS"),
                       ("iPhone", "XR"),
                       ("IPHONEOS","XROS")])

        # Setup TBB build configs for iOS Simulator
        if context.buildTarget in [apple_utils.TARGET_IOS_SIMULATOR_ARM64, apple_utils.TARGET_IOS_SIMULATOR_X86_64]:
            # Create iOS simulator config from iOS config
            shutil.copy(
                src="build/ios.macos.inc",
                dst="build/iossimulator.macos.inc")

            PatchFile("build/iossimulator.macos.inc",
                      [("iphoneos", "iphonesimulator")])

            shutil.copy(src="build/macos.clang.inc",
                        dst="build/iossimulator.clang.inc")

            # Patch to use iOS simulator deployment target flag
            PatchFile("build/iossimulator.clang.inc",
                      [("ifeq (ios,$(target))", "ifeq (iossimulator,$(target))"),
                       ("-miphoneos-version-min=", "-mios-simulator-version-min=")])

        # Setup TBB build configs for visionOS Simulator
        if context.buildTarget in [apple_utils.TARGET_VISIONOS_SIMULATOR_ARM64, apple_utils.TARGET_VISIONOS_SIMULATOR_X86_64]:
            # Create visionOS simulator config from iOS config
            shutil.copy(
                src="build/ios.macos.inc",
                dst="build/visionossimulator.macos.inc")

            PatchFile("build/visionossimulator.macos.inc",
                      [("iphoneos", "xrsimulator"),
                       ("ios", "visionossimulator"),
                       ("iOS", "visionOS Simulator"),
                       ("iPhone", "XR Simulator")])

            shutil.copy(src="build/macos.clang.inc",
                        dst="build/visionossimulator.clang.inc")

            # Patch for visionOS simulator
            # Use -target flag with architecture mapping (same pattern as visionOS device)
            # The version from the original flag will be appended to create: -target arch-apple-xros-simulator<version>
            PatchFile("build/visionossimulator.clang.inc",
                      [("ifeq (ios,$(target))", "# Map TBB's intel64 to x86_64 for target triple\nVISIONOS_ARCH := $(subst intel64,x86_64,$(arch))\n\nifeq (visionossimulator,$(target))"),
                       ("-miphoneos-version-min=", "-target $(VISIONOS_ARCH)-apple-xros-simulator")])
        ...
        def _RunBuild(arch):
            if not arch:
                return
            env = os.environ.copy()
            # Use static TBB (big_iron.inc) for all our static build targets
            if apple_utils.TargetNeedsStaticTBB(context):
                env["SDKROOT"] = apple_utils.GetSDKRoot(context)

                # Determine target name for TBB makefile
                target_name = context.buildTarget.lower().replace("-", "")

                # Map our targets to TBB target names
                if apple_utils.TargetIsMacOSStatic(context):
                    target_name = "macos"  # Use standard macos target for macOS static builds
                elif context.buildTarget in [apple_utils.TARGET_IOS_SIMULATOR_ARM64, apple_utils.TARGET_IOS_SIMULATOR_X86_64]:
                    target_name = "iossimulator"
                elif context.buildTarget in [apple_utils.TARGET_VISIONOS_SIMULATOR_ARM64, apple_utils.TARGET_VISIONOS_SIMULATOR_X86_64]:
                    target_name = "visionossimulator"
                elif context.buildTarget in [apple_utils.TARGET_VISIONOS_ARM64]:
                    target_name = "visionos"

                # Determine architecture for TBB build
                tbb_arch = apple_utils.GetTargetArchFromBuildTarget(context) or arch
                if tbb_arch == apple_utils.TARGET_X86:
                    tbb_arch = "intel64"  # TBB uses different arch name

                buildArgs.append(f' compiler=clang arch={tbb_arch} extra_inc=big_iron.inc target={target_name}')
...
def InstallJPEG(context, force, buildArgs):
    with CurrentWorkingDirectory(DownloadURL(JPEG_URL, context, force)):
        # CUSTOM PATCH: Fix CMake string command for iOS cross-compilation
        # libjpeg-turbo 2.0.1 has a bug where CMAKE_SYSTEM_PROCESSOR can be empty,
        # causing string(TOLOWER ${CMAKE_SYSTEM_PROCESSOR} ...) to fail
        PatchFile("CMakeLists.txt", [
        ...
        extraJPEGArgs = [
            "-DBUILD_SHARED_LIBS=OFF",
            "-DCMAKE_POLICY_VERSION_MINIMUM=3.5",
            "-DENABLE_SHARED=OFF",  # CUSTOM PATCH: Disable libjpeg-turbo shared libs
            "-DENABLE_STATIC=ON",   # CUSTOM PATCH: Enable static libs only
            "-DCMAKE_MACOSX_BUNDLE=OFF",  # CUSTOM PATCH: Disable bundle for iOS cross-compilation
        ] + buildArgs
        if not which("nasm"):
...
def InstallTIFF(context, force, buildArgs):
    with CurrentWorkingDirectory(DownloadURL(TIFF_URL, context, force)):
        ...
        if MacOS() or Windows():
            ...
            extraArgs = []
        extraArgs.append("-DBUILD_SHARED_LIBS=OFF")  # CUSTOM PATCH: Static build
        extraArgs.append("-DCMAKE_POLICY_VERSION_MINIMUM=3.5")  # CUSTOM PATCH: CMake compatibility
        extraArgs += buildArgs
...
def InstallPNG(context, force, buildArgs):
    with CurrentWorkingDirectory(DownloadURL(PNG_URL, context, force)):
        ...
        macArgs = ["-DPNG_FRAMEWORK=OFF", "-DBUILD_SHARED_LIBS=OFF", "-DCMAKE_POLICY_VERSION_MINIMUM=3.5", "-DPNG_EXECUTABLES=OFF"]  # CUSTOM PATCH: Static build + CMake compatibility + disable executables for iOS/visionOS
    ...
    with CurrentWorkingDirectory(DownloadURL(OPENEXR_URL, context, force)):
        RunCMake(context, force,
                 ['-DOPENEXR_INSTALL_TOOLS=OFF',
                  ...
                  '-DBUILD_TESTING=OFF',
                  '-DBUILD_SHARED_LIBS=OFF',
                  '-DCMAKE_POLICY_VERSION_MINIMUM=3.5'] + buildArgs)  # CUSTOM PATCH: Static build + CMake compatibility
...
def InstallOpenImageIO(context, force, buildArgs):
    with CurrentWorkingDirectory(DownloadURL(OIIO_URL, context, force)):
        # CUSTOM PATCH: Fix CMake if() command for iOS cross-compilation
        # OpenImageIO 2.5.16 has a bug where CMAKE_SYSTEM_PROCESSOR can be empty,
        # causing if() statements with unquoted variables to fail
...
def InstallDraco(context, force, buildArgs):
    with CurrentWorkingDirectory(DownloadURL(DRACO_URL, context, force)):
        cmakeOptions = [
            '-DBUILD_SHARED_LIBS=OFF',  # Build static library for static USD builds
        ]
        cmakeOptions += buildArgs
        RunCMake(context, force, cmakeOptions)
...
def InstallMaterialX(context, force, buildArgs):
    with CurrentWorkingDirectory(DownloadURL(MATERIALX_URL, context, force)):
        ...
        if MacOSTargetEmbedded(context) or apple_utils.TargetIsSimulator(context) or apple_utils.TargetIsMacOSStatic(context):
            ...
            # Set MATERIALX_BUILD_IOS for iOS and simulators
            if apple_utils.TargetIsSimulator(context) or context.buildTarget == apple_utils.TARGET_IOS:
                cmakeOptions.append('-DMATERIALX_BUILD_IOS=ON')
...
class InstallContext:
    def __init__(self, args):
        ...
        self.buildTools = (args.build_tools and 
                           not embedded and 
                           not self.targetWasm)
        # Allow validation on embedded platforms if explicitly requested
        # Original logic disabled validation for embedded platforms, but we want
        # consistency across iOS device and simulators
        ...
        # CUSTOM PATCH: Allow OpenImageIO for embedded targets (iOS/visionOS)
...
# CUSTOM PATCH: OpenImageIO can be built without Imaging
if context.buildOIIO:
...
# STATIC BUILD OVERRIDE: For static monolithic builds, we force zlib to be built
# to ensure we have a static libz.a for complete static linking without runtime
# dependencies on system libraries, even if no features that typically require
# zlib are enabled.
if context.buildMonolithic and ZLIB not in requiredDependencies:
...
#
# For monolithic/static builds, we keep zlib in the dependencies (added above).
if ZLIB in requiredDependencies:
    if context.targetWasm or not context.buildZlib:
        requiredDependencies = [r for r in requiredDependencies if r != ZLIB]
    elif (Linux() or MacOS()) and not context.buildMonolithic:
        # Only skip zlib for dynamic builds on Linux/macOS
        # For monolithic/static builds, we need our own static zlib
        requiredDependencies = [r for r in requiredDependencies if r != ZLIB]
...
if MacOSTargetEmbedded(context):
    ...
    if "--tools" in sys.argv:
        ...
        sys.exit(1)
    # CUSTOM PATCH: Allow OpenImageIO for embedded targets (iOS/visionOS)
    # OpenImageIO and its dependencies will be built as static libraries
